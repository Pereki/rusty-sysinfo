...
impl Dial {
    ...
    pub fn render_dial(percentage: &f32, diameter: &i32) -> Vec<Vec<char>> {
        let angle = percentage / 100.00 * 180.00;
        ...
        Dial::render_outer_border(&mut canvas, &radius, &center_x, &center_y);
    ...
    fn render_outer_border(
        ...
        for ang in 0..180 {
            ...
            let border_x = center_x
                + (border_radians.cos() * Dial::SCALE_MULTIPLIER * radius.to_owned() as f32).round()
                    as i32;
            let border_y =
                center_y - (border_radians.sin() * radius.to_owned() as f32).round() as i32;
